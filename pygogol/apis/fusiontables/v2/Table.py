...
def refetchSheet(tableId=None):
    url = baseUrl + "tables/{tableId}/refetch"
    method = "POST"
    return Request(
        method, 
        url.format(['tableId']) + "?" + "&".join(),
...
def replaceRows(tableId=None, delimiter=None, encoding=None, endLine=None, isStrict=None, startLine=None):
    url = baseUrl + "tables/{tableId}/replace"
    method = "POST"
    return Request(
        method, 
        url.format(['tableId']) + "?" + "&".join(),
