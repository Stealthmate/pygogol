...
def aggregatedList(project=None, filter=None, maxResults=None, orderBy=None, pageToken=None):
    url = baseUrl + "{project}/aggregated/commitments"
    method = "GET"
    return Request(
        method, 
...
def get(commitment=None, project=None, region=None):
    url = baseUrl + "{project}/regions/{region}/commitments/{commitment}"
    method = "GET"
    return Request(
        method, 
        url.format(['commitment', 'project', 'region']) + "?" + "&".join(),
        {},
        jsonDumps()
    )

def insert(project=None, region=None, requestId=None):
    url = baseUrl + "{project}/regions/{region}/commitments"
    method = "POST"
    return Request(
        method, 
